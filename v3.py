...
def gen_phrases(gen_rules, els_dict, els_arr, max_phrases_per_rule):
			...
			for fld_rule in rule:
				...
				if ii == 0: # i rule
					if sel_el != None or els_set == [] or els_set[1] == None or els_set[1] <= 1:
						numrecs *= 1
					elif df_type == df_type.varobj or df_type == df_type.obj:
						numrecs *= els_set[1]
					else:
						logger.error('Invalid field ID combination for numrecs calc. Exiting')
						exit()
	...
	ivec_pos_list = [ivec_pos_list[i] for i in shuffle_stick]
...
v_r_eval = tf.Variable(tf.random_shuffle(tf.range(numrecs)))
